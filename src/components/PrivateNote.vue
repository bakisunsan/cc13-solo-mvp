<template>
  <div class="privateNote">
    ...
    <div id="chat-form">
      ...
      />
      <button class="submit" v-on:click="createPrivateNote()">
        Put It! 🤏
      </button>
    </div>
      ...
      <div v-for="(privateNote, id) in privateNotes" v-bind:key="id">
        <div class="private-note" :color="randomColor(privateNote.content)">
          <div>
            ...
            <a @click="() => deletePrivateNote(privateNote.id)"
...
export default {
  ...
  mounted: function() {
    this.setOwner().then(this.displayPrivateNotes());
  },
  methods: {
    ...
    createPrivateNote: async function() {
      ...
      try {
        this.content = "";
        await API.graphql(
          graphqlOperation(createPrivateNote, { input: privateNote })
        );
      } catch (error) {
        error;
      }
    ...
    deletePrivateNote: async function(id) {
      ...
      try {
        this.content = "";
        await API.graphql(
          graphqlOperation(deletePrivateNote, { input: { id: id } })
        );
      } catch (error) {
        error;
      }
    ...
    displayPrivateNotes: async function() {
      let privateNotes = await API.graphql(
        graphqlOperation(listPrivateNotes, { limit: this.limit })
      );
      this.privateNotes = _.orderBy(
        privateNotes.data.listPrivateNotes.items,
        "updatedAt",
        "desc"
      ).slice(0, 100);

      API.graphql(
        graphqlOperation(onCreatePrivateNote, {
          limit: this.limit,
          owner: this.owner
        })
      ).subscribe({
      ...
      API.graphql(
        graphqlOperation(onDeletePrivateNote, {
          limit: this.limit,
          owner: this.owner
        })
      ).subscribe({
        next: eventData => {
          ...
          const privateNotes = this.privateNotes.filter(
            x => x.id !== privateNote.id
          );
          this.privateNotes = _.orderBy(
            privateNotes,
            "updatedAt",
    ...
    singlePrivateNote: async function(selectedNote) {
      let privateNote = await API.graphql(
        graphqlOperation(getPrivateNote, { id: selectedNote.id })
      );
...
.private-notes-area {
  ...
  background: linear-gradient(white, white),
    linear-gradient(#4d4d4d, #4d4d4d) no-repeat 100% 100% / 30px 30px,
    linear-gradient(#4d4d4d, #4d4d4d) no-repeat 0 100% / 30px 30px,
    linear-gradient(#4d4d4d, #4d4d4d) no-repeat 100% 0 / 30px 30px,
    linear-gradient(#4d4d4d, #4d4d4d) no-repeat 0 0 / 30px 30px,
